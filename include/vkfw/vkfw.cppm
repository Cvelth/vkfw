...
#include <GLFW/glfw3.h>
  ...
  #include <version>
#endif
#if !defined(VKFW_NO_STRING_VIEW)
  ...
  #endif
#endif
#if !defined(VKFW_NO_SPACESHIP_OPERATOR) && defined(__cpp_impl_three_way_comparison)               \
  && defined(__has_include) && 201711 <= __cpp_impl_three_way_comparison && __has_include(<compare> )
...
#if !defined(VKFW_NO_NODISCARD_WARNINGS)
...
import std.compat;
  ...
  export namespace VKFW_NAMESPACE { enum Boolean { VKFW_ENUMERATOR(True) = GLFW_TRUE, VKFW_ENUMERATOR(False) = GLFW_FALSE };
...
template <> struct InitializationHintTraits<InitializationHint::VKFW_ENUMERATOR(WaylandLibDecor)> {
...
template <> struct AttributeTraits<Attribute::VKFW_ENUMERATOR(MousePassthrough)> : std::true_type {
  using type = bool;
};
...
template <> struct AttributeTraits<Attribute::VKFW_ENUMERATOR(ContextRevision)> : std::false_type {
  using type = size_t;
};
template <>
