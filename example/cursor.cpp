...
public:
	...
	void on_key(vkfw::Window const &window, vkfw::Key key,
...
void CursorExample::on_move(vkfw::Window const &, double x, double y) {
	printf("%0.3f: Cursor position: %f %f (%+f %+f)\n",
		   vkfw::getTime(), x, y, x - cursor_x, y - cursor_y);
	cursor_x = x;
	cursor_y = y;
}
void CursorExample::on_key(vkfw::Window const &window, vkfw::Key key,
						   vkfw::KeyAction action, vkfw::ModifierKeyFlags mods) {
	if (action == vkfw::KeyAction::ePress)
		switch (key) {
			...
			case vkfw::Key::eDown:
			{
				...
				window.setCursorPos(static_cast<double>(width - 1),
			...
			case vkfw::Key::e1:
				window.setCursor(*standard_cursors[0]);
				break;

			case vkfw::Key::e2:
				window.setCursor(*standard_cursors[1]);
				break;

			case vkfw::Key::e3:
				window.setCursor(*standard_cursors[2]);
				break;

			case vkfw::Key::e4:
				window.setCursor(*standard_cursors[3]);
				break;

			case vkfw::Key::e5:
				window.setCursor(*standard_cursors[4]);
				break;

			case vkfw::Key::e6:
				window.setCursor(*standard_cursors[5]);
				break;
...
CursorExample::CursorExample() {
	...
	std::ranges::generate(standard_cursors, []() {
		...
		constexpr std::array shapes = {
			vkfw::CursorShape::eArrow,
			vkfw::CursorShape::eIBeam,
			vkfw::CursorShape::eCrosshair,
			vkfw::CursorShape::eHand,
			vkfw::CursorShape::eHResize,
			vkfw::CursorShape::eVResize
		};
...
int CursorExample::run() {
	...
	window->callbacks()->on_cursor_move =
		[this](auto const &w, auto x, auto y) { on_move(w, x, y); };
	window->callbacks()->on_key =
		[this](auto const &w, auto k, auto, auto a, auto m) { on_key(w, k, a, m); };
	...
	while (!window->shouldClose()) {
		...
		if (track_cursor) {
			...
			glViewport(0, 0,
					   static_cast<GLsizei>(framebuffer_width),
