...
void key_callback(vkfw::Window const &window, vkfw::Key key, int scancode, 
...
void key_callback(vkfw::Window const &window, vkfw::Key key, int, 
				  vkfw::KeyAction action, vkfw::ModifierKeyFlags mods) {
	...
	if ((key == vkfw::Key::eEnter && mods == vkfw::ModifierKeyBits::eAlt) ||
		...
		if (window.getMonitor()) {
			window.setMonitor(nullptr, windowed_xpos, windowed_ypos,
							  static_cast<size_t>(windowed_width),
			...
			if (monitor) {
				...
				window.setMonitor(monitor, 0, 0,
								  static_cast<size_t>(video_mode->width),
